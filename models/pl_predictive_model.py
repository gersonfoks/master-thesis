...
class GaussianPredictiveModelPL(pl.LightningModule):
...
class MLEPredictiveModelPL(pl.LightningModule):
    ...
    def __init__(self, nmt_model, tokenizer, padding_id=-100, device="cuda", lr=0.00005, weight_decay=1e-5):
        ...
        self.linear_layers = nn.Sequential(nn.Linear(512 * 2, 512), torch.nn.SiLU(), nn.Dropout(p=0.25), nn.Linear(512, 256), torch.nn.SiLU(), nn.Dropout(p=0.25), nn.Linear(256, 128), torch.nn.SiLU(), nn.Dropout(p=0.25),
                                           nn.Linear(128, 1))
        self.linear_layers.to("cuda")
    ...
    def predict(self, sources, targets):
        ...
        predicted_risk = self.forward(input_ids, attention_mask, labels, decoder_input_ids)
