...
class BIListmModelWithRef(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_size=512, hidden_size=512):
        ...
        self.reference_lstm_layer = torch.nn.LSTM(embedding_size, hidden_size=hidden_size, num_layers=self.num_layers,
                                        bidirectional=bool(self.bidirectional), batch_first=True, dropout=self.dropout)
    ...
    def create_linear_layers(self):
        ...
        layers.append(nn.Tanh())# Append a tanh layer (to make sure to bound the output values)

        return nn.Sequential(*layers)


    def forward(self, model_input, ref_input, best_score):
        x_embedded = self.element_wise_apply(self.embedding, model_input)
        ref_embed = self.element_wise_apply(self.embedding, ref_input)

        output, (h_n, c_n) = self.lstm_layer(x_embedded)
        output, (h_n_ref, c_n) = self.reference_lstm_layer(ref_embed)

        h_n = h_n.permute(1,0, 2)
        h_n = h_n.reshape(-1, self.hidden_size * self.multiply_factor)

        h_n_ref = h_n_ref.permute(1, 0, 2)
        h_n_ref = h_n_ref.reshape(-1, self.hidden_size * self.multiply_factor)


        #linear_layer_input = torch.cat([h_n, h_n_ref, best_score.reshape(-1, 1)], dim=-1)
        linear_layer_input = torch.cat([h_n, h_n_ref], dim=-1)
        predicted_score = self.linear_layers(linear_layer_input)
...
class PlLSTMModelWithRef(pl.LightningModule):
    ...
    def forward(self, input_ids, ref_input_ids, best_score):
        return self.model.forward(input_ids, ref_input_ids, best_score)
    ...
    def batch_to_out(self, batch):
        ...
        input_ids, ref_input_ids, best_score, score = batch

        predicted_score = self.forward(input_ids, ref_input_ids, best_score)
